...
// Optional authored math plugins support the user's requested LaTeX report.
// Existing callers retain the original Markdown/editor behavior.
export function RichNarrative({ id, value, sourcePreviews, remarkPlugins = [], rehypePlugins = [], className = "", label = "Narrative", ...attributes }) {
  ...
  return <div {...attributes} className={["rich-narrative", className].filter(Boolean).join(" ")}
    ...
    </LexicalComposer> : <div className="rich-narrative-content"><Markdown remarkPlugins={[...markdownPlugins, ...remarkPlugins]} rehypePlugins={rehypePlugins} components={markdownComponents}>{markdown}</Markdown></div>}
  </div>;
}
