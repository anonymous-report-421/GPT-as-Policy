...
export function DataAppShell({
  ...
  // Explicitly requested for this formal evaluation report, 2026-09-13.
  // Other app identities retain the standard chrome and its behavior.
  const omitReportToolbar = reportSurface && snapshot.id === "report:ea6bbbe4-7b8a-4e94-8dcb-c4901c4d10c7";
  useInputModality();
    ...
    <DataAppContext.Provider value={shellContext}>
      <DataAppBlockLayoutContext.Provider value={blockLayoutContext}>
        <DashboardAskProvider
          ...
          enabled={!omitReportToolbar && canUseDashboardAsk({ canEdit: ownerCanEdit, mode })}
          explorationEnabled={mode === "view"}
          dashboardTitle={appTitle}
          onStatus={setActionStatus}
        >
          <div className="dashboard-root" style={omitReportToolbar ? { "--dashboard-topbar-base-height": "0px" } : undefined}>
            {!omitReportToolbar && <DataAppTopbar
              title={appTitle}
