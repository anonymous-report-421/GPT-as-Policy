...
function StandardChartRenderer({
  ...
  const categoryWidth = horizontal
    ? Math.max(54, Number.isFinite(categoryAxisMinWidth) ? categoryAxisMinWidth : 0, Math.min(126, maximumValue(rows.map((row) => categoryLabel(x, row[x] ?? "").length * 8 + 18), 0)))
    : numericAxisWidth;
  ...
  const fixedValueDomain = Array.isArray(spec.valueDomain) && spec.valueDomain.length === 2
    && spec.valueDomain.every(Number.isFinite) && spec.valueDomain[0] < spec.valueDomain[1]
    ? spec.valueDomain : null;
  const valueDomain = fixedValueDomain ?? (spec.startAtZero === false ? ["dataMin", "auto"] : zeroAnchoredDomain);
  ...
  const yAxis = (
    <ValueYAxis
      ...
      tick={hideYAxisCategoryTicks ? false : horizontal
        ? renderCategoryTick
          ? (props) => renderCategoryTick({ ...props, field: x, width: categoryWidth, side: categoryAxisPosition })
  ...
  const xAxis = (
    <ValueXAxis
      ...
      tickCount={horizontal && (benchmarkDomain || fixedValueDomain) ? 6 : horizontal && !proportional ? 4 : undefined}
          ...
          >
            ...
            {(type === "leaderboard" || spec.showValues) && (
              <LabelList
                ...
                formatter={(value) => spec.valueDecimals != null ? formatTooltipValue(value, field) : formatMarkValue(value, field)}
                fill={spec.valueLabelColor}
                fontSize={spec.valueLabelFontSize}
